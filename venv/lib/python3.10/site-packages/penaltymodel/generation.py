...
class Index:
    ...
    #AE: changed 3 parts + own print to 32 bits
def all_possible(num_variables: int) -> np.ndarray:
    ...
    #AE change: np.uint8 to np.uint32 and added view(np.uint8)
...
def generate(graph_like: GraphLike,
    ...
    while True:
        ...
        if res.success:
            ...
            #if len(auxiliary_state)==3: # AE inserted to test
              #  print("ground:", ground)
            i = ground[decision_state][auxiliary_state]
            upper_bound.remove(i)
            equality.append(i)
